...
public class UserService {
    ...
    public void createUser(User user) {
        if (user.getPassword().isEmpty() || user.getUsername().isEmpty()){
            throw new EmptyFieldException("Not all fields are filled in!");
        }
    ...
    public User getUserById(Long id){
        return userRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("The user with the specified id was not found"));
    }

    public List<User> getAllByName(String username){
        var users =  userRepository.findAllByUsername(username);
        if (users.isEmpty()) {
            throw new NotFoundException("No users with the specified name were found");
        }
        return users;
    }
    ...
    public void checkUserCredentials(User user, String username, String password) {
        if (!user.getPassword().equals(password) || !user.getUsername().equals(username)){
            throw new WrongDataException("Incorrect username and/or password!");
        }
    }
